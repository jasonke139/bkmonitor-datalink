...
func (c Config) Validate() error {
	if c.PluginName == "" {
		return types.JoinErrors(types.ErrInvalidConfig(), errors.New("plugin name is empty"))
	}

	if c.PluginVersion == "" {
		return types.JoinErrors(types.ErrInvalidConfig(), errors.New("plugin version is empty"))
	}

	if c.ReconnectInterval == 0 {
		return types.JoinErrors(types.ErrInvalidConfig(), errors.New("reconnect interval is 0"))
	}

	if c.KeepaliveInterval == 0 {
		return types.JoinErrors(types.ErrInvalidConfig(), errors.New("keepalive interval is 0"))
	}

	if c.RecvCallback == nil {
		return types.JoinErrors(types.ErrInvalidConfig(), errors.New("recv callback function is empty"))
	}

	if c.Logger == nil {
		return types.JoinErrors(types.ErrInvalidConfig(), errors.New("logger is empty"))
	}
